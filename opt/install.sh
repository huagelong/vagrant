...
yum -y install vim
tar xf ./nginx-1.10.2.tar.gz -C /usr/src/ && cd /usr/src/nginx-1.10.2/
...
\cp -rf ./nginx.conf /srv/nginx/conf/
...
tar xf ./php-7.1.0.tar.gz -C /usr/src/ && cd /usr/src/php-7.1.0/
...
make -j4 && make install
\cp -rf ./php.ini /srv/php/lib/php.ini
\cp -rf ./php-fpm.conf /srv/php/etc/php-fpm.conf
\cp -rf ./www.conf /srv/php/etc/php-fpm.d/www.conf
...
tar xf ./swoole-src-4.2.1.tar.gz -C /usr/src/ && cd /usr/src/swoole-src-4.2.1/
/srv/php/bin/phpize
