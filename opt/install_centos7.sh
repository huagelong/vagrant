...
tar xf $(pwd)/nginx-1.10.2.tar.gz -C /usr/src/ && cd /usr/src/nginx-1.10.2/
./configure --prefix=/opt/nginx --with-http_ssl_module --with-http_gzip_static_module --with-http_stub_status_module --with-pcre
make -j4 && make install
\cp -rf $(pwd)/nginx /etc/init.d/nginx
chmod a+x /etc/init.d/nginx
\cp -rf $(pwd)/nginx.conf /opt/nginx/conf/
...
tar xf $(pwd)/php-7.1.0.tar.gz -C /usr/src/ && cd /usr/src/php-7.1.0/
...
make -j4 && make install
\cp -rf $(pwd)/php.ini /opt/php/lib/php.ini
\cp -rf $(pwd)/php-fpm.conf /opt/php/etc/php-fpm.conf
\cp -rf $(pwd)/www.conf /opt/php/etc/php-fpm.d/www.conf
mkdir -p /opt/php/var/session
mkdir -p /opt/php/log/
echo 'export PATH="/opt/php/bin:$PATH"'>>/etc/profile
source /etc/profile
\cp -rf $(pwd)/composer /usr/local/bin/composer
chmod a+x /usr/local/bin/composer
\cp -rf /usr/src/php-7.1.0/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm
chmod a+x /etc/init.d/php-fpm
tar xf $(pwd)/swoole-src-4.2.1.tar.gz -C /usr/src/ && cd /usr/src/swoole-src-4.2.1/
/opt/php/bin/phpize
...
cd /vagrant/opt
tar xf $(pwd)/mysql.tar.gz -C /usr/src/ && cd /usr/src/mysql-386776d/
phpize
./configure
make -j4 && make install
